...
with st.form("bulk_upload_form"):
    ...
    if retrain_button and bulk_files:
        with st.spinner("Uploading images and retraining model..."):
            from src.prediction import retrain_model
            from src.model import save_model
            success_count = 0
            label_dir = os.path.join("data/custom", class_label)
            ...
            for file in bulk_files:
                try:
                    ...
                    st.warning(f"Error saving {file.name}: {e}")
            if success_count:
                try:
                    new_model = retrain_model("data/custom")
                    save_model(new_model, "models/best_model.h5")
                    st.success(f"Uploaded {success_count} images for class '{class_label}' and retrained successfully.")
                except Exception as e:
